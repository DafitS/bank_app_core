...
from bank_app.domain.repositories.operation_history_repository import (
    OperationHistoryRepository,
)
from bank_app.infrastructure.orm.operation_history import (
    OperationHistory as OperationHistoryORM,
)
...
class SqlAlchemyOperationHistoryRepository(OperationHistoryRepository):
    ...
    def get_by_id(self, operation_id: UUID) -> OperationHistory:
        ...
        operation_history_searched = (
            self.session.query(OperationHistoryORM)
                .filter(OperationHistoryORM.operation_id == operation_id)
                .first())
    ...
    def get_by_account_id(self, account_id: UUID):
        ...
        operations_searched = (self.session.query(OperationHistoryORM)
                                .filter(OperationHistoryORM.account_id == account_id)
                                .all())
